...
import { selectSources76, askDrByte76 } from './drbyte76-model';
import './drbyte76.css';
...
export function Assistant72({onClose,userId,moduleName,supabase,loadPdfJs,lectureCatalog=[]}) {
  ...
  const upload=useRef(null),body=useRef(null),alive=useRef(true),locked=useRef(false),urls=useRef(new Set()),loadingPdf=useRef(null),streamRef=useRef(null),requestRef=useRef(null);
  ...
  async function openLibrary(){if(locked.current)return;locked.current=true;setBusy('Henter dit bibliotek…');setNotice('');try{
    ...
    const rows=[...(lecture.data||[]).filter(r=>r.mime_type==='application/pdf'||/\.pdf$/i.test(r.file_name)).map(r=>{const lecture=lectureCatalog.find(l=>l.id===r.lecture_id);return {id:`lecture-${r.id}`,title:r.file_name,sourceTitle:[lecture?.id||r.lecture_id,lecture?.title,r.file_name].filter(Boolean).join(' · '),path:r.storage_path,bucket:'lecture-materials'};}),...(exam.data||[]).flatMap(r=>[{id:`exam-${r.id}`,title:r.question_file_name||r.file_name,path:r.question_storage_path||r.storage_path,bucket:'exam-set-documents'},...(r.answer_storage_path?[{id:`answer-${r.id}`,title:r.answer_file_name||'Facit.pdf',path:r.answer_storage_path,bucket:'exam-set-documents'}]:[])])].filter(r=>r.path);
  ...
  async function send(e){e.preventDefault();const q=question.trim();if(!q||busy||locked.current)return;
    locked.current=true;setNotice('');setBusy(webSearch?'Søger på nettet og læser dine uddrag…':'Dr. Byte læser og svarer…');
    const controller=new AbortController();requestRef.current=controller;const timeout=setTimeout(()=>controller.abort(),60000);
    const sources=selectSources76(docs,q);const history=messages.slice(-6).map(m=>({role:m.role,text:m.text.slice(0,3000)}));
    try{const result=await askDrByte76({question:q,sources,history,context:context?.text||'',screen:screen||'',web:webSearch},supabase,controller.signal);
      if(!alive.current||controller.signal.aborted)return;
      setMessages(m=>[...m,{role:'user',text:q+(context?'\nAppvisning vedhæftet':'')+(screen?'\nSkærmbillede vedhæftet':'')},{role:'assistant',...result}]);
      setQuestion('');setContext(null);setScreen(null);
      if(webSearch&&!result.web?.searched)setNotice('Google returnerede ingen websøgning til dette spørgsmål. Svaret er ikke markeret som webkontrolleret.');
  ...
  return <section className="mf72 mf72-assistant" aria-label="Dr. Byte"><header className="mf72-chat-head"><span className="mf72-byte-icon" aria-hidden="true">✧</span><div><h2>Dr. Byte</h2><small>Dit pensum, tæt på.</small></div><button type="button" aria-label="Luk Dr. Byte" onClick={onClose}>×</button></header>
    <div className="mf72-chat-body" ref={body}><div className="mf72-chat-welcome"><h3>Hvad vil du forstå?</h3><p>Spørg til dit pensum. Vælg PDF’er i Bibliotek, eller tilføj dine egne. Kildeknapper åbner den konkrete PDF-side.</p><small>Der søges i tekstuddrag — ikke automatisk i hele pensum. PDF-side og trykt slidenummer kan være forskellige.</small></div>
      ...
      {messages.map((m,i)=><article key={i} className="mf72-chat-message mf76-message" data-role={m.role}>{m.paragraphs?m.paragraphs.map((p,j)=><div key={j}><p>{p.text}</p>{p.citations.length>0&&<div className="mf76-citations">{p.citations.map(s=>s.kind==='web'?<a key={s.id} href={s.url} target="_blank" rel="noopener noreferrer">{s.id} · {s.title} ↗</a>:<details key={s.id}><summary>{s.title} · PDF-side {s.page}</summary><blockquote>{s.text}</blockquote><button type="button" onClick={()=>openSource(s)}>Åbn side {s.page} ↗</button></details>)}</div>}</div>):m.text}{m.web?.searched&&<details className="mf76-web"><summary>Websøgning · {m.web.sources.length} kilder</summary>{m.web.sources.map(s=><a key={s.id} href={s.url} target="_blank" rel="noopener noreferrer">{s.title} ↗</a>)}{m.web.suggestions&&<iframe title="Søgeforslag fra Google" sandbox="allow-popups allow-popups-to-escape-sandbox" referrerPolicy="no-referrer" srcDoc={m.web.suggestions}/>}</details>}{m.role==='assistant'&&<small className="mf76-footnote">Gemini · Kontrollér vigtige oplysninger i originalkilden.</small>}</article>)}
      {busy&&<p className="mf72-muted" role="status">{busy}</p>}{notice&&<p className="mf72-error" role="alert">{notice}</p>}
    </div><form className="mf72-chat-compose" onSubmit={send}><div className="mf72-row"><button type="button" disabled={!!busy} onClick={attachContext}>▣ Se min visning</button><button type="button" disabled={!!busy} onClick={()=>upload.current.click()}>+ PDF</button><button type="button" disabled={!!busy||!userId} onClick={openLibrary}>Bibliotek</button>{navigator.mediaDevices?.getDisplayMedia&&<button type="button" disabled={!!busy} onClick={captureScreen} title="Vælg selv hvilken fane eller skærm der skal fotograferes">Skærmbillede</button>}<button type="button" disabled={!!busy} aria-pressed={webSearch} onClick={()=>setWebSearch(v=>!v)}>Søg på nettet {webSearch?'✓':''}</button></div><input ref={upload} type="file" accept="application/pdf" hidden multiple onChange={e=>{importFiles([...e.target.files]);e.target.value='';}}/>
      ...
      <textarea aria-label="Spørg Dr. Byte" maxLength={6000} readOnly={!!busy} placeholder="Spørg til en forelæsning eller et begreb…" value={question} onChange={e=>setQuestion(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey&&!e.nativeEvent.isComposing){e.preventDefault();send(e);}}}/><small className="mf76-privacy">Når du sender, deles dit spørgsmål, valgte uddrag og vedhæftninger med Google Gemini. Ingen patientoplysninger. {webSearch&&'Websøgning deler kun selve spørgsmålet med søgetjenesten.'}</small><div className="mf72-row"><small style={{flex:1}}>Gemini · {docs.filter(d=>d.selected).length} PDF’er valgt</small>{requestRef.current&&<button type="button" onClick={()=>requestRef.current?.abort()}>Stop</button>}<button type="submit" disabled={!question.trim()||!!busy||!userId} className="mf72-primary">Send ↗</button></div>
...
const GUIDE72=[
  ...
  {title:'Kilder tæt på dit spørgsmål.',intro:'Dr. Byte svarer med Gemini ud fra valgte PDF-uddrag, din vedhæftede appvisning og valgfri websøgning. Kildeknapper åbner PDF-siden. Du bestemmer, hvad der deles med Google. Del ikke patientoplysninger.',tag:'DR. BYTE & HJÆLP',rows:[['PDF-bibliotek','Vælg dine kilder'],['Se min visning','Kun når du vælger det'],['Søg på nettet','Et valgfrit supplement']]},
];
const FAQ72=[
