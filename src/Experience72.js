...
export function Assistant72({onClose,userId,moduleName,supabase,loadPdfJs,lectureCatalog=[],activeLecture=null}) {
  const [docs,setDocs]=useState([]),[question,setQuestion]=useState(''),[messages,setMessages]=useState([]),[context,setContext]=useState(null),[screen,setScreen]=useState(null),[busy,setBusy]=useState(''),[notice,setNotice]=useState(''),[library,setLibrary]=useState(null),[chosen,setChosen]=useState([]),[viewer,setViewer]=useState(null),[manualLectureId,setManualLectureId]=useState(null),[lecturePicker,setLecturePicker]=useState(false),[quizPicker,setQuizPicker]=useState(false),[quizSettings,setQuizSettings]=useState({format:'mcq',count:3,flow:'guided'});
  ...
  async function send(e){e.preventDefault();const q=question.trim();if(!q||busy||locked.current)return;
    ...
    try{const sources=await sourcesFor(q);const result=await askDrByte76({question:q,sources,history,context:context?.text||'',screen:screen||'',web:false},supabase,controller.signal);
      if(!alive.current||controller.signal.aborted)return;
      setMessages(m=>[...m,{role:'user',lectureId:focusedLectureId,text:q+(context?'\nAppvisning vedhæftet':''),screen:screen||null},{role:'assistant',lectureId:focusedLectureId,...result}]);
  ...
  async function generateQuiz(rawSettings=quizSettings,previous=null){if(busy||locked.current)return;let settings;try{settings=normalizeQuizSettings78(rawSettings);}catch(error){setNotice(error.message);return;}setQuizPicker(false);locked.current=true;setNotice('');setBusy('Dr. Byte laver spørgsmål…');const controller=new AbortController();requestRef.current=controller;const timeout=setTimeout(()=>controller.abort(),60000);
    try{
      ...
      const result=await requestQuizBatches78(settings,sources,async payload=>askDrByte76({question:q,...payload,context:context?.text||'',screen:screen||'',web:false},supabase,controller.signal),batch=>{accumulated.push(...batch);update({questions:[...accumulated]});},previous);
  ...
  return <section className="mf72 mf72-assistant mf77-assistant" aria-label="Dr. Byte"><header className="mf72-chat-head"><span className="mf72-byte-icon" aria-hidden="true">✧</span><div><h2>Dr. Byte</h2><small>Dit pensum, tæt på.</small></div><button type="button" aria-label="Luk Dr. Byte" onClick={onClose}>×</button></header>
    ...
    </form>{viewer&&<div role="dialog" aria-modal="true" aria-label="PDF-kilde" className="mf72-pdf-view"><header><strong>{viewer.title} · side {viewer.page}</strong><a href={`${viewer.url}#page=${viewer.page}`} target="_blank" rel="noreferrer">Åbn separat</a><button type="button" onClick={closeSource}>Luk kilde</button></header><iframe title={viewer.title} src={`${viewer.url}#page=${viewer.page}`} style={{width:'100%',height:'100%',border:0}}/></div>}
  </section>;
}
