...
function Quiz77({quiz,onSource}) {
  const [answers,setAnswers]=useState({}),[revealed,setRevealed]=useState({});
  return <section className="mf77-quiz" aria-label="Spørgsmål fra Dr. Byte"><header><strong>{quiz.format==='mcq'?'Multiple choice':'Kortsvar'}</strong><small>{quiz.questions.length} spørgsmål · baseret på valgte PDF-uddrag</small></header>{quiz.questions.map((item,index)=><div className="mf77-quiz-item" key={index}><span className="mf77-quiz-number">{String(index+1).padStart(2,'0')}</span><h4>{item.prompt}</h4>{quiz.format==='mcq'?<div className="mf77-quiz-options">{item.options.map((option,n)=><button key={n} type="button" disabled={answers[index]!=null} data-result={answers[index]!=null?(n===item.answerIndex?'correct':n===answers[index]?'incorrect':undefined):undefined} onClick={()=>setAnswers(current=>({...current,[index]:n}))}><span>{String.fromCharCode(65+n)}</span>{option}</button>)}</div>:<><textarea aria-label={`Dit svar på spørgsmål ${index+1}`} placeholder="Skriv dit svar med egne ord…" value={answers[index]||''} onChange={event=>setAnswers(current=>({...current,[index]:event.target.value}))}/><button type="button" disabled={!answers[index]?.trim()} onClick={()=>setRevealed(current=>({...current,[index]:true}))}>Se forslag til svar</button></>}{(quiz.format==='mcq'?answers[index]!=null:revealed[index])&&<div className="mf77-quiz-feedback"><strong>{quiz.format==='mcq'?(answers[index]===item.answerIndex?'Korrekt':'Se forklaringen'):'Forslag til svar'}</strong>{quiz.format==='short'&&<p>{item.modelAnswer}</p>}<p>{item.explanation}</p><button type="button" onClick={()=>onSource(item.source)}>Åbn {item.source.title} · PDF-side {item.source.page} ↗</button></div>}</div>)}</section>;
}

export function Assistant72({onClose,userId,moduleName,supabase,loadPdfJs,lectureCatalog=[],activeLecture=null}) {
  const [docs,setDocs]=useState([]),[question,setQuestion]=useState(''),[messages,setMessages]=useState([]),[context,setContext]=useState(null),[screen,setScreen]=useState(null),[busy,setBusy]=useState(''),[notice,setNotice]=useState(''),[library,setLibrary]=useState(null),[chosen,setChosen]=useState([]),[viewer,setViewer]=useState(null),[manualLectureId,setManualLectureId]=useState(null),[lecturePicker,setLecturePicker]=useState(false),[quizPicker,setQuizPicker]=useState(false);
  ...
  async function generateQuiz(format){if(busy||locked.current)return;setQuizPicker(false);locked.current=true;setNotice('');setBusy('Dr. Byte laver spørgsmål…');const controller=new AbortController();requestRef.current=controller;const timeout=setTimeout(()=>controller.abort(),60000);
    try{const q=question.trim()||`Lav spørgsmål til ${focusedLecture?.title||'den valgte forelæsning'}`;const sources=await sourcesFor(q);if(!sources.length)throw Error('Vælg en forelæsning med en læsbar PDF, eller tilføj en PDF, før du genererer spørgsmål.');const result=await askDrByte76({question:q,sources,mode:'quiz',quizFormat:format,context:context?.text||'',screen:screen||'',web:false},supabase,controller.signal);if(!alive.current||controller.signal.aborted)return;setMessages(current=>[...current,{role:'assistant',quiz:result.quiz}]);setQuestion('');setContext(null);setScreen(null);
  ...
  return <section className="mf72 mf72-assistant mf77-assistant" aria-label="Dr. Byte"><header className="mf72-chat-head"><span className="mf72-byte-icon" aria-hidden="true">✧</span><div><h2>Dr. Byte</h2><small>Dit pensum, tæt på.</small></div><button type="button" aria-label="Luk Dr. Byte" onClick={onClose}>×</button></header>
    <div className="mf72-chat-body" ref={body}><div className="mf72-chat-welcome"><h3>Hvad vil du forstå?</h3><p>{focusedLecture?<>Jeg bruger <strong>{focusedLecture.title}</strong>, når du spørger. Du kan vælge en anden forelæsning nedenfor.</>:'Spørg frit, eller vælg en forelæsning, så Dr. Byte kan bruge dens PDF.'}</p></div>
      ...
      {quizPicker&&<div className="mf77-picker mf77-quiz-picker"><p>Vælg format. Spørgsmålene bliver lavet ud fra forelæsningens PDF.</p><button type="button" onClick={()=>generateQuiz('mcq')}>Multiple choice</button><button type="button" onClick={()=>generateQuiz('short')}>Kortsvar</button></div>}
