...
import { createActiveClock72, sanitizeRich72, richText72, textHtml72, validMasks72, activityGeometry73, readingIndex73 } from './experience72-model';
import './experience72.css';
import { selectSources76, selectLectureDocs76, selectConversationHistory76, associateStoredLectureDocs76, loadLectureMaterials76, askDrByte76 } from './drbyte76-model';
import { normalizeQuizSettings78, requestQuizBatches78 } from './quiz78-model';
import { DrByte79Rail, useDrByteHistory79 } from './DrByte79';
import { DrByteTone79 } from './DrByteTone79';
import { toneSettings79 } from './drbyte79-tone';
import { ByteSource79 } from './ByteSource79';
import { sourcePreview79 } from './drbyte79-retrieval';
import './drbyte76.css';
...
export function AreaTabs72({area,activeTab,onSelect,language,Icon}) {
  ...
  const tabs=area==='training' ? [['training-start',en?'Flashcards':'Flashkort','training','training-history'],['training-exams',en?'Exam sets':'Eksamenssæt','file','training-exam-history']] : area==='curriculum' ? [['curriculum-lectures',en?'Lectures':'Forelæsninger','curriculum']] : area==='planning' ? [['planning-calendar',en?'Calendar':'Kalender','calendar'],['planning-study-plan',en?'Study plan':'Studieplan','planning']] : [];
...
function ByteIcon77({name}) {
  const shared={width:17,height:17,viewBox:'0 0 24 24',fill:'none',stroke:'currentColor',strokeWidth:1.8,strokeLinecap:'round',strokeLinejoin:'round','aria-hidden':true};
  if(name==='screen')return <svg {...shared}><rect x="3" y="4" width="18" height="14" rx="2"/><path d="M8 21h8M12 18v3"/><circle cx="17" cy="8" r="1" fill="currentColor" stroke="none"/></svg>;
  if(name==='lecture')return <svg {...shared}><path d="M4 5.5c3-1.5 5-1.3 8 .3v14c-3-1.6-5-1.8-8-.3zM20 5.5c-3-1.5-5-1.3-8 .3v14c3-1.6 5-1.8 8-.3z"/><path d="M7 9h2m-2 3h2m6-3h2"/></svg>;
  return <svg {...shared}><rect x="4" y="5" width="16" height="15" rx="3"/><path d="M8 10h8M8 14h5M9 2v3m6-3v3"/><path d="m17 14 1.4 1.4L21 12.8"/></svg>;
}

function Quiz78({quiz,onSource,onRetry}) {
  const [answers,setAnswers]=useState({}),[revealed,setRevealed]=useState({}),[hints,setHints]=useState({}),[index,setIndex]=useState(0);
  const guided=quiz.flow!=='questions-only';
  const visible=guided?quiz.questions.slice(index,index+1).map((item)=>[item,index]):quiz.questions.map((item,i)=>[item,i]);
  return <section className="mf77-quiz mf78-quiz" aria-label="Spørgsmål fra Dr. Byte">
    <header><strong>{quiz.format==='mcq'?'Multiple choice':'Kortsvar'}</strong><small>{quiz.questions.length}/{quiz.requestedCount||quiz.questions.length} spørgsmål · {guided?'Guidet trin for trin':'Kun spørgsmål'}</small></header>
    {visible.map(([item,n])=>{
      const id=item.id||String(n),isRevealed=Boolean(revealed[id]),answer=answers[id];
      return <div className="mf77-quiz-item" key={id}>
        <span className="mf77-quiz-number">{String(n+1).padStart(2,'0')}</span><h4>{item.prompt}</h4>
        {quiz.format==='mcq'?<div className="mf77-quiz-options">{item.options.map((option,k)=><button key={k} type="button" aria-pressed={answer===k} data-result={isRevealed?(k===item.answerIndex?'correct':k===answer?'incorrect':undefined):undefined} onClick={()=>setAnswers(current=>({...current,[id]:k}))}><span>{String.fromCharCode(65+k)}</span>{option}</button>)}</div>:<textarea aria-label={`Dit svar på spørgsmål ${n+1}`} placeholder="Skriv dit svar med egne ord…" value={answer||''} onChange={event=>setAnswers(current=>({...current,[id]:event.target.value}))}/>}
        <div className="mf78-quiz-actions">{guided&&item.hint&&!isRevealed&&!hints[id]&&<button type="button" onClick={()=>setHints(current=>({...current,[id]:true}))}>Vis et hint</button>}{!isRevealed&&<button type="button" onClick={()=>setRevealed(current=>({...current,[id]:true}))}>Vis svar</button>}</div>
        {guided&&hints[id]&&!isRevealed&&<p className="mf78-quiz-hint">{item.hint}</p>}
        {isRevealed&&<div className="mf77-quiz-feedback"><strong>{quiz.format==='mcq'&&answer!=null?(answer===item.answerIndex?'Dit valg er korrekt':'Se det korrekte svar'):'Forslag til svar'}</strong><p>{quiz.format==='mcq'?item.options[item.answerIndex]:item.modelAnswer}</p><p>{item.explanation}</p><button type="button" onClick={()=>onSource(item.source)}>Åbn {item.source.title} · PDF-side {item.source.page} ↗</button></div>}
      </div>;
    })}
    {guided&&quiz.questions.length>1&&<div className="mf78-quiz-navigation"><button type="button" disabled={index===0} onClick={()=>setIndex(value=>value-1)}>Forrige</button><span>{index+1} / {quiz.questions.length}</span><button type="button" disabled={index>=quiz.questions.length-1} onClick={()=>setIndex(value=>value+1)}>Næste</button></div>}
    {quiz.error&&<div className="mf78-quiz-partial" role="status"><p>{quiz.questions.length} spørgsmål er bevaret. Resten kunne ikke hentes endnu.</p><button type="button" onClick={onRetry}>Prøv resten igen</button></div>}
  </section>;
}

export function Assistant72({onClose,userId,moduleName,supabase,loadPdfJs,lectureCatalog=[],activeLecture=null}) {
  const {messages,setMessages,question,setQuestion,activeId,history,syncState,openConversation,beginConversation,persistExchange,renameConversation,deleteConversation}=useDrByteHistory79(supabase,userId);
  const [docs,setDocs]=useState([]),[context,setContext]=useState(null),[screen,setScreen]=useState(null),[busy,setBusy]=useState(''),[notice,setNotice]=useState(''),[library,setLibrary]=useState(null),[chosen,setChosen]=useState([]),[viewer,setViewer]=useState(null),[manualLectureId,setManualLectureId]=useState(null),[lecturePicker,setLecturePicker]=useState(false),[quizPicker,setQuizPicker]=useState(false),[quizSettings,setQuizSettings]=useState({format:'mcq',count:3,flow:'guided'}),[historyOpen,setHistoryOpen]=useState(false),[fullScreen,setFullScreen]=useState(false);
  const [toneOpen,setToneOpen]=useState(false);
  const [toneSettings,setToneSettings]=useState(()=>{try{return toneSettings79(JSON.parse(localStorage.getItem(`medfluen-drbyte79-tone:${userId}`)||'null'));}catch{return toneSettings79(null);}});
  const upload=useRef(null),body=useRef(null),alive=useRef(true),locked=useRef(false),urls=useRef(new Set()),loadingPdf=useRef(null),streamRef=useRef(null),requestRef=useRef(null),loadedLectures=useRef(new Set());
  useEffect(()=>()=>requestRef.current?.abort(),[]);
  useEffect(()=>{try{localStorage.setItem(`medfluen-drbyte79-tone:${userId}`,JSON.stringify(toneSettings79(toneSettings)));}catch{/* Browser storage is optional. */}},[userId,toneSettings]);
  useEffect(()=>{alive.current=true;documentStore72(userId,'read').then(rows=>{if(alive.current)setDocs(rows.map(r=>({...r,selected:r.selected!==false})));}).catch(()=>{if(alive.current)setNotice('Browseren kan ikke gemme PDF-biblioteket. Du kan stadig bruge det i denne åbne chat.');});return()=>{alive.current=false;loadingPdf.current?.destroy();streamRef.current?.getTracks().forEach(t=>t.stop());urls.current.forEach(url=>URL.revokeObjectURL(url));};},[userId]);
  useEffect(()=>{body.current?.scrollTo({top:body.current.scrollHeight,behavior:'auto'});},[messages.length]);
  async function indexFile(file,id,metadata={}){
  ...
  async function importLibrary(){if(locked.current)return;locked.current=true;setNotice('');try{for(const item of (library||[]).filter(r=>chosen.includes(r.id))){setBusy(`Åbner ${item.title}…`);const {data,error}=await supabase.storage.from(item.bucket).download(item.path);if(error)throw Error(`${item.title} kunne ikke hentes.`);await indexFile(new File([data],item.title,{type:'application/pdf'}),item.id,item);}if(alive.current)setLibrary(null);}catch(e){if(alive.current)setNotice(e.message);}finally{locked.current=false;if(alive.current)setBusy('');}}
  const focusedLectureId=manualLectureId||activeLecture?.id||null;
  const focusedLecture=lectureCatalog.find(row=>row.id===focusedLectureId)||(manualLectureId?null:activeLecture);
  async function sourcesFor(q){
    let scoped=selectLectureDocs76(docs,activeLecture?.id,manualLectureId);
    if(focusedLectureId&&!loadedLectures.current.has(focusedLectureId)){
      setBusy('Henter forelæsningens PDF…');
      const {data,error}=await supabase.from('lecture_materials').select('id,file_name,storage_path,mime_type,lecture_id').eq('module_name',moduleName).eq('lecture_id',focusedLectureId);
      if(error){if(!scoped.some(doc=>doc.lectureId===focusedLectureId))throw Error('Forelæsningens materialer kunne ikke hentes. Prøv igen.');return selectSources76(scoped,q);}
      const materials=(data||[]).filter(row=>row.storage_path&&(row.mime_type==='application/pdf'||/\.pdf$/i.test(row.file_name))).slice(0,3);
      const upgraded=associateStoredLectureDocs76(materials,docs,focusedLectureId);
      for(const doc of upgraded){try{await documentStore72(userId,'put',doc);}catch{/* The current chat can still use the PDF. */}}
      if(upgraded.length){setDocs(current=>current.map(doc=>upgraded.find(row=>row.id===doc.id)||doc));scoped=[...scoped,...upgraded.filter(doc=>doc.selected)];}
      const loaded=await loadLectureMaterials76(materials,docs,async item=>{const {data:blob,error:downloadError}=await supabase.storage.from('lecture-materials').download(item.storage_path);if(downloadError)throw Error(`${item.file_name} kunne ikke hentes.`);return {item,blob};},async ({item,blob})=>indexFile(new File([blob],item.file_name,{type:'application/pdf'}),`lecture-${item.id}`,{lectureId:focusedLectureId,sourceTitle:[focusedLecture?.id,focusedLecture?.title,item.file_name].filter(Boolean).join(' · ')}));
      scoped=[...scoped,...loaded.added];
      if(loaded.failures&&!scoped.length)throw Error('Forelæsningens PDF’er kunne ikke læses. Kontrollér at mindst én har et tekstlag, eller tilføj en læsbar PDF.');
      if(loaded.failures)setNotice(`${loaded.failures} PDF-filer kunne ikke læses. Jeg bruger kun de læsbare kilder.`);
      loadedLectures.current.add(focusedLectureId);
    }
    if(focusedLectureId&&!scoped.length)throw Error('Ingen læsbare PDF-kilder er valgt til denne forelæsning. Vælg dens PDF i chatten eller tilføj en læsbar PDF.');
    return selectSources76(scoped,q);
  }
  ...
  async function send(e){e.preventDefault();const q=question.trim();if(!q||busy||locked.current)return;
    locked.current=true;setNotice('');setBusy('Dr. Byte læser og svarer…');
    const controller=new AbortController();requestRef.current=controller;const timeout=setTimeout(()=>controller.abort(),60000);
    const history=selectConversationHistory76(messages,focusedLectureId);
    try{const sources=await sourcesFor(q);const result=await askDrByte76({question:q,sources,history,context:context?.text||'',screen:screen||'',web:false,tone:toneSettings.tone,customStyle:toneSettings.custom},supabase,controller.signal);
      if(!alive.current||controller.signal.aborted)return;
      const userMessage={role:'user',lectureId:focusedLectureId,text:q,screen:screen||null};
      const assistantMessage={role:'assistant',lectureId:focusedLectureId,...result};
      setMessages(m=>[...m,userMessage,assistantMessage]);
      setQuestion('');setContext(null);setScreen(null);
      try{await persistExchange(userMessage,assistantMessage);}catch{if(alive.current)setNotice('Svaret vises, men samtalen kunne ikke synkroniseres. Kontrollér, at SQL-filen til samtalehistorik er kørt.');}
    }catch(error){if(alive.current)setNotice(controller.signal.aborted?'Forespørgslen blev stoppet eller tog for lang tid. Dit spørgsmål er bevaret.':error.message);}
    finally{clearTimeout(timeout);requestRef.current=null;locked.current=false;if(alive.current)setBusy('');}
  }
  async function generateQuiz(rawSettings=quizSettings,previous=null){if(busy||locked.current)return;let settings;try{settings=normalizeQuizSettings78(rawSettings);}catch(error){setNotice(error.message);return;}setQuizPicker(false);locked.current=true;setNotice('');setBusy('Dr. Byte laver spørgsmål…');const controller=new AbortController();requestRef.current=controller;const timeout=setTimeout(()=>controller.abort(),60000);
    try{
      const q=question.trim()||`Lav spørgsmål til ${focusedLecture?.title||'den valgte forelæsning'}`;
      const sources=await sourcesFor(q);
      if(!sources.length)throw Error('Vælg en forelæsning med en læsbar PDF, eller tilføj en PDF, før du genererer spørgsmål.');
      const id=previous?.id||`quiz-${Date.now()}-${Math.random().toString(36).slice(2,8)}`;
      if(!previous)setMessages(current=>[...current,{role:'assistant',quiz:{id,format:settings.format,flow:settings.flow,requestedCount:settings.count,questions:[]}}]);
      const update=patch=>setMessages(current=>current.map(message=>message.quiz?.id===id?{...message,quiz:{...message.quiz,...patch}}:message));
      const accumulated=[...(previous?.questions||[])];
      const result=await requestQuizBatches78(settings,sources,async payload=>askDrByte76({question:q,...payload,context:context?.text||'',screen:screen||'',web:false,tone:toneSettings.tone,customStyle:toneSettings.custom},supabase,controller.signal),batch=>{accumulated.push(...batch);update({questions:[...accumulated]});},previous);
      if(!alive.current)return;
      update({questions:result.questions,error:result.error?.message||''});
      if(result.error)setNotice(`Kun ${result.completed} af ${settings.count} spørgsmål blev lavet. De er bevaret; prøv resten igen senere.`);
      else {setQuestion('');setContext(null);setScreen(null);}
    }catch(error){if(alive.current)setNotice(controller.signal.aborted?'Spørgsmålene tog for lang tid. Prøv igen.':error.message);}finally{clearTimeout(timeout);requestRef.current=null;locked.current=false;if(alive.current)setBusy('');}}
  function openSource(source){const d=docs.find(d=>d.id===source.documentId);if(!d){setNotice('Kilden er fjernet fra biblioteket. Tilføj den igen for at åbne siden.');return;}const page=d.pages?.find(row=>Number(row.page)===Number(source.page));const verified=sourcePreview79(source,page);const url=URL.createObjectURL(new Blob([d.bytes],{type:'application/pdf'}));urls.current.add(url);setViewer({url,page:source.page,title:d.title,quote:verified?.quote||null});}
  ...
  function chooseDoc(doc,selected){const next={...doc,selected};setDocs(rows=>rows.map(row=>row.id===doc.id?next:row));documentStore72(userId,'put',next).catch(()=>setNotice('PDF-valget kunne ikke gemmes i browseren. Det gælder kun i denne åbne chat.'));}
  const railProps={history,activeId,onNew:()=>{beginConversation();setHistoryOpen(false);},onOpen:async id=>{try{await openConversation(id);setHistoryOpen(false);}catch{setNotice('Samtalen kunne ikke åbnes. Prøv igen.');}},onRename:async(id,title)=>{try{await renameConversation(id,title);}catch{setNotice('Titlen kunne ikke gemmes.');}},onDelete:async id=>{try{await deleteConversation(id);}catch{setNotice('Samtalen kunne ikke slettes.');}},syncState};
  return <section className={`mf72 mf72-assistant mf77-assistant${fullScreen?' mf79-byte-fullscreen':''}`} aria-label="Dr. Byte">
    {fullScreen&&<DrByte79Rail {...railProps}/>}
    <header className="mf72-chat-head"><span className="mf72-byte-icon" aria-hidden="true">✧</span><div><h2>Dr. Byte</h2><small>Dit pensum, tæt på.</small></div><div className="mf79-byte-head-actions"><button type="button" onClick={()=>{setToneOpen(value=>!value);setHistoryOpen(false);}} aria-expanded={toneOpen} aria-label="Svarstil">Svarstil</button><button type="button" onClick={()=>{setHistoryOpen(value=>!value);setToneOpen(false);}} aria-expanded={historyOpen} aria-label="Samtalehistorik">Historik</button><button type="button" onClick={()=>setFullScreen(value=>!value)} aria-label={fullScreen?'Luk fuldskærm':'Åbn Dr. Byte i fuldskærm'}>{fullScreen?'Luk fuldskærm':'Fuldskærm'}</button><button type="button" aria-label="Luk Dr. Byte" onClick={onClose}>×</button></div></header>
    {historyOpen&&<div className="mf79-byte-history-popover"><DrByte79Rail {...railProps}/></div>}
    {toneOpen&&<DrByteTone79 settings={toneSettings} onChange={setToneSettings} onClose={()=>setToneOpen(false)}/>}
    <div className="mf72-chat-body" ref={body}><div className="mf72-chat-welcome"><h3>Hvad vil du forstå?</h3><p>{focusedLecture?<>Jeg bruger <strong>{focusedLecture.title}</strong>, når du spørger. Du kan vælge en anden forelæsning nedenfor.</>:'Spørg frit, eller vælg en forelæsning, så Dr. Byte kan bruge dens PDF.'}</p></div>
      {docs.length>0&&<details className="mf72-library"><summary>{docs.filter(d=>d.selected).length} af {docs.length} PDF’er valgt</summary>{docs.map(doc=><div key={doc.id} className="mf72-row"><label style={{flex:1}}><input type="checkbox" checked={doc.selected} onChange={e=>chooseDoc(doc,e.target.checked)}/><span>{doc.title}<br/><small>{doc.pages.length} sider med tekst</small></span></label><button type="button" aria-label={`Fjern ${doc.title}`} onClick={()=>removeDoc(doc)}>×</button></div>)}<p className="mf72-muted">Gemt lokalt for din bruger på denne enhed. Fjern filer her, hvis de ikke skal blive liggende.</p></details>}
      ...
      {messages.map((m,i)=><article key={i} className="mf72-chat-message mf76-message" data-role={m.role}>{m.quiz?<Quiz78 quiz={m.quiz} onSource={openSource} onRetry={()=>generateQuiz({format:m.quiz.format,count:m.quiz.requestedCount,flow:m.quiz.flow},m.quiz)}/>:m.paragraphs?m.paragraphs.map((p,j)=><div key={j}><p>{p.text}</p>{p.citations.length>0&&<div className="mf76-citations">{p.citations.map(s=>s.kind==='web'?<a key={s.id} href={s.url} target="_blank" rel="noopener noreferrer">{s.id} · {s.title} ↗</a>:<details key={s.id}><summary>{s.title} · PDF-side {s.page}</summary><blockquote>{s.text}</blockquote><button type="button" onClick={()=>openSource(s)}>Åbn side {s.page} ↗</button></details>)}</div>}</div>):m.text}{m.screen&&<img className="mf77-chat-attachment" src={m.screen} alt="Vedhæftet skærmbillede"/>}{m.web?.searched&&<details className="mf76-web"><summary>Websøgning · {m.web.sources.length} kilder</summary>{m.web.sources.map(s=><a key={s.id} href={s.url} target="_blank" rel="noopener noreferrer">{s.title} ↗</a>)}{m.web.suggestions&&<iframe title="Søgeforslag fra Google" sandbox="allow-popups allow-popups-to-escape-sandbox" referrerPolicy="no-referrer" srcDoc={m.web.suggestions}/>}</details>}{m.role==='assistant'&&!m.quiz&&<small className="mf76-footnote">Gemini · Kontrollér vigtige oplysninger i originalkilden.</small>}</article>)}
      {busy&&<p className="mf72-muted" role="status">{busy}</p>}{notice&&<p className="mf72-error" role="alert">{notice}</p>}
    </div><form className="mf72-chat-compose" onSubmit={send}><div className="mf77-chat-tools"><button type="button" disabled={!!busy||!navigator.mediaDevices?.getDisplayMedia} onClick={captureScreen} title="Vælg fanen med MedFLUEN eller en anden skærm"><ByteIcon77 name="screen"/>Skærmbillede</button><button type="button" disabled={!!busy} onClick={()=>{setLecturePicker(value=>!value);setQuizPicker(false);}} aria-expanded={lecturePicker}><ByteIcon77 name="lecture"/>Vælg forelæsning</button><button type="button" disabled={!!busy} onClick={()=>{setQuizPicker(value=>!value);setLecturePicker(false);}} aria-expanded={quizPicker}><ByteIcon77 name="quiz"/>Generer spørgsmål</button></div><input ref={upload} type="file" accept="application/pdf" hidden multiple onChange={e=>{importFiles([...e.target.files]);e.target.value='';}}/>
      {lecturePicker&&<div className="mf77-picker"><label htmlFor="mf77-lecture-choice">Forelæsning</label><select id="mf77-lecture-choice" value={manualLectureId||''} onChange={e=>{setManualLectureId(e.target.value||null);setLecturePicker(false);}}><option value="">Automatisk{activeLecture?` · ${activeLecture.title}`:''}</option>{lectureCatalog.map(row=><option key={row.id} value={row.id}>{row.id} · {row.title}</option>)}</select><button type="button" onClick={()=>upload.current.click()}>Tilføj egen PDF</button><button type="button" disabled={!userId} onClick={openLibrary}>PDF-bibliotek</button><button type="button" onClick={attachContext}>Del synlig apptekst</button></div>}
      {quizPicker&&<div className="mf77-picker mf78-quiz-settings"><p>Vælg selv, hvordan du vil øve dig. Spørgsmålene bygger på den valgte forelæsnings PDF.</p><fieldset><legend>Format</legend>{[['mcq','Multiple choice'],['short','Kortsvar']].map(([value,label])=><label key={value}><input type="radio" name="byte-quiz-format" checked={quizSettings.format===value} onChange={()=>setQuizSettings(current=>({...current,format:value}))}/>{label}</label>)}</fieldset><fieldset><legend>Antal spørgsmål</legend>{[1,3,5,10].map(value=><label key={value}><input type="radio" name="byte-quiz-count" checked={quizSettings.count===value} onChange={()=>setQuizSettings(current=>({...current,count:value}))}/>{value}</label>)}</fieldset><fieldset><legend>Forløb</legend>{[['questions-only','Kun spørgsmål','Alle spørgsmål samlet. Facit skjules, indtil du vælger at se det.'],['guided','Guidet trin for trin','Ét spørgsmål ad gangen med valgfrit hint og forklaring.']].map(([value,label,help])=><label key={value} className="mf78-quiz-flow"><input type="radio" name="byte-quiz-flow" checked={quizSettings.flow===value} onChange={()=>setQuizSettings(current=>({...current,flow:value}))}/><span><strong>{label}</strong><small>{help}</small></span></label>)}</fieldset><small className="mf78-quiz-source">Kilde: {focusedLecture?.title||'valgte læsbare PDF’er'}. {quizSettings.count>3?'Større sæt hentes i mindre dele og bruger flere af dagens gratis forespørgsler.':''}</small><button type="button" className="mf72-primary" onClick={()=>generateQuiz()}>Generer {quizSettings.count} spørgsmål</button></div>}
      ...
      <textarea aria-label="Spørg Dr. Byte" maxLength={6000} readOnly={!!busy} placeholder="Spørg til en forelæsning eller et begreb…" value={question} onChange={e=>setQuestion(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey&&!e.nativeEvent.isComposing){e.preventDefault();send(e);}}}/><div className="mf77-compose-foot"><small>{focusedLecture?`Forelæsning · ${focusedLecture.title}`:'Ingen forelæsning valgt'} · {selectLectureDocs76(docs,activeLecture?.id,manualLectureId).length} PDF’er</small><button type="button" onClick={()=>upload.current.click()}>+ PDF</button>{requestRef.current&&<button type="button" onClick={()=>requestRef.current?.abort()}>Stop</button>}<button type="submit" disabled={!question.trim()||!!busy||!userId} className="mf72-primary">Send ↗</button></div><small className="mf76-privacy">Kun når du sender, deles spørgsmål, valgte uddrag og vedhæftninger med Google Gemini. Del ikke patientoplysninger.</small>
    </form>{viewer&&<ByteSource79 title={viewer.title} page={viewer.page} quote={viewer.quote} url={viewer.url} onClose={closeSource}/>}
  </section>;
}
...
const GUIDE72=[
  ...
  {title:'Kilder tæt på dit spørgsmål.',intro:'Dr. Byte svarer med Gemini ud fra valgte PDF-uddrag og din vedhæftede appvisning. Kildeknapper åbner PDF-siden. Du bestemmer, hvad der deles med Google. Del ikke patientoplysninger.',tag:'DR. BYTE & HJÆLP',rows:[['PDF-bibliotek','Vælg dine kilder'],['Se min visning','Kun når du vælger det'],['Kilder','Åbn den konkrete PDF-side']]},
];
const FAQ72=[
  ...
  ['Hvorfor mangler der tekst fra en PDF?','Tekstbaserede PDF’er kan søges lokalt. Scannede sider uden tekstlag kræver OCR, som ikke er inkluderet. PDF’er over 25 MB eller 1.000 sider skal deles op. Gemini får højst 12 udvalgte tekstuddrag, ikke hele biblioteket.'],
  ['Kan Dr. Byte se min skærm automatisk?','Nej. Se min visning vedhæfter synlig apptekst. Skærmbillede lader dig vælge en skærm eller fane. Gennemse og fjern vedhæftningen efter behov. Når du trykker Send, deles den med Google Gemini sammen med spørgsmålet og valgte tekstuddrag.'],
  ['Hvor opbevares PDF’er i Dr. Byte?','PDF-biblioteket gemmes lokalt for din bruger på denne enhed. Udvalgte tekstuddrag sendes til Google, når du stiller et spørgsmål. Googles datavilkår gælder for det sendte indhold. Del ikke patientoplysninger eller fortroligt materiale. Fjern filer fra biblioteket efter behov.'],
