...
export function SlideNotes74({ annotations = [], fileName = "PDF", materialId, page = 1, numPages = 0, status = "ready", onSave, onDelete, onRetry, onPage, onClose, language = "da" }) {
  ...
  return <section className="mf74-notes" aria-label={en ? "Slide notes" : "Slidenoter"}>
    ...
    {mode === "recall" ? <div className="mf74-recall">
      ...
      </div> : <div className="mf74-empty"><span className="mf74-empty-icon">✎</span><h3>{en ? "Space for your thoughts" : "Plads til dine tanker"}</h3><p>{en ? "Create a note for the selected slide. Notes are saved here, separately from the PDF file." : "Opret en note til det valgte slide. Noter gemmes her, separat fra selve PDF-filen."}</p><button type="button" disabled={!materialId} onClick={add}>{en ? "Write a slide note" : "Skriv en slidenote"}</button></div>}
